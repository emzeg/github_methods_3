...
The model indicates that for the PAS effect, participants were less frequently rating their stimuli with higher scores, ???!!!which is why a slope for pas isn't really being modelled.!!!???
...
Between m7 and m7.1, we believe that the m7 is the best model, as model m7.1 is a poor model. In m7.1, neither pas nor task explains count score to any significance, whereas in m7 both tasks and the interaction between pas and tasks significantly explains count score.

   
(vi)  
m7 tells us that there is an important interaction between pas and task when it comes to the distribution of ratings. 
...
```
When we add PAS as a main effect on top of task, we can see that PAS actually explains more of the variance (19%) than task (0.2%) and is a statistically significant predictor of performance. This would indicate that task does not need to be included in the model.
```{r ex3p3iii}
